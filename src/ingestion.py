...
def _carregar_markdown(caminho: Path) -> List[Document]:
    # TextLoader é suficiente aqui: nosso Markdown é texto simples,
    # não precisamos de um parser HTML/NLP pesado (evita dependências
    # externas como o UnstructuredMarkdownLoader, que baixa modelos NLTK
    # da internet e pode falhar em redes corporativas/restritas).
...
def carregar_documentos(pasta: Path = DOCS_DIR) -> List[Document]:
    ...
    for arquivo in sorted(pasta.iterdir()):
        ...
        if loader_fn is None:
            print(f"[ingestion] Formato não suportado, ignorando: {arquivo.name}")
            continue

        docs = loader_fn(arquivo)
        categoria = CATEGORIA_POR_ARQUIVO.get(arquivo.name, "Não categorizado")
        ...
        print(f"[ingestion] {arquivo.name}: {len(docs)} documento(s) carregado(s) — categoria '{categoria}'")

    print(f"[ingestion] Total de documentos carregados: {len(documentos)}")
    return documentos
